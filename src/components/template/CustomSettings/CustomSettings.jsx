...
import { userPatchUpdatePasswordMutation } from "@/service/user";
...
export default function CustomSettings() {
    ...
    const { mutate: mutation, isPending } = useCustomMutation({
        mutationFn: userPatchUpdatePasswordMutation,
        invalidateKeys: ["user-settings"],
        successMessage: "Parol o'zgartirildi!",
        onSuccess: () => {
            reset();
        },
    });

    const onSubmit = (data) => mutation(data);
        ...
        <div className="w-full max-w-[700px]  mx-auto ">
            ...
            <div className="w-auto p-4 text-center border border-1 m-5 rounded-xl">
                ...
                <p className="text-textGrey mt-5 ">
                    Click the camera icon to upload or capture a new profile
                    photo
                </p>
            </div>
    ...
    bg-gradient-to-r from-orange-500 to-red-600 hover:opacity-90 transition"
                    >
                        {isPending ? "Update Password..." : "Update Passwordte"}
                    </button>
                </form>
                <div className="w-auto  text-center border border-1 m-5 rounded-xl">
                    <div className="mb-[20px]">
                        <CustomIcon
                            ...
                            iconColor={"text-orange-500"}
                            className={
                                "font-bold border-none  bg-orange-900 ml-2 "
                            }
                        />
                    </div>
                    <p className="text-textColor text-[10px] w-full max-w-[550px] mt-[-20px] text-start ml-auto">
                        For security reasons, please use a strong password that
                        includes uppercase and lowercase letters, numbers, and
                        special characters. Make sure to keep your account
                        information secure and avoid sharing your credentials
                        with others.
                    </p>
                </div>
            </div>
